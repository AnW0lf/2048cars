...
public class Table : MonoBehaviour
{
    ...
    {
        int minX = (size - tableSize) / 2;
        int maxX = (size + tableSize) / 2;
        ...
        int minDistance = (size - tableSize) / 2;
        int maxDistance = size - tableSize + 1;
    ...
    private bool CheckPushAnotherUnit(Unit unit)
    {
        ...
        if (Math.Max(first.Y, second.Y) >= (size + tableSize) / 2 - 1) return false;
    ...
    private void PushAnotherUnit(Unit unit)
    {
        ...
        if (Math.Max(first.Y, second.Y) >= (size + tableSize) / 2) return;
    ...
    private bool GameOver
    {
        get
        {
            int min = (size - tableSize) / 2;
            int max = (size + tableSize) / 2;
            for(int i = 0; i < cells.Length; i++)
            {
                int x = i % size, y = i / size;
                if (this[x, y] != null && (x < min || x >= max || y < min || y >= max))
                    return true;
            }
            return false;
        }
